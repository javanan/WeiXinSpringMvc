...
public class SysFilter extends OncePerRequestFilter {
	...
	public void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (admin != null || PatternMatchUtils.simpleMatch(excepts, reqUri)
				|| (reqUri.contains(".") && !reqUri.toLowerCase().endsWith(".jsp"))) {
			// 把request保存在线程局部变量中
			...
			if (request.getHeader("x-requested-with") != null
				...
				if (aulous == null) {
					...
					response.sendRedirect(request.getContextPath() + "/admin/adminLoginSpark?username=" + aulous
							+ "&pwd=0&noVali=true");
				}
			}
		}
